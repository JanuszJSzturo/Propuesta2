...
# img = dcm.pixel_array
# img2 = img[0,:,:]
# plt.imshow(image_icbm[70,:,:], cmap=plt.cm.bone)
# plt.show()
#
# plt.imshow(image_RM_Brain_0[70,:,:], cmap=plt.cm.bone)
# plt.show()
# print(dcm)
...
parametros_iniciales = [0, 0, 0,
                        np.pi/4, 1, 0, 0,  # Inicializamos el eje de la rotacion a un vector unitario
                        0, 0, 0]
...
final_pos = [transformacion_rigida_3D(voxel, parametros_iniciales) for voxel in init_pos]

final_pos = np.array(final_pos).astype(int)
final_pos = np.round(final_pos)

new_image = np.zeros(image_icbm.shape)
d, h, w = image_icbm.shape
...
for i, voxel_pos in enumerate(final_pos):
    ...
    if new_d >= 0 and new_d < d and new_h >= 0 and new_h < h and new_w >= 0 and new_w < w:
        index = init_pos[i]
        value_gray = image_icbm[tuple(index)]
        new_image[tuple(voxel_pos)] = value_gray

plt.imshow(new_image[70,:,:], cmap=plt.cm.bone)
plt.show()
